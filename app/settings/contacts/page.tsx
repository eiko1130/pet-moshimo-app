...
export default function ContactsPage() {
  ...
  const handleOpenQr = async () => {
    if (!displayId) return
    const appUrl = 'https://pet-moshimo-app.vercel.app'
    const url = await QRCode.toDataURL(`${appUrl}/auth?ref=${displayId}`, { width: 300, margin: 2 })
    setQrDataUrl(url)
    setShowQrModal(true)
  }
            ...
            {displayId && (
              <div className="bg-pink-50 rounded-2xl p-4 mt-2">
                <p className="text-xs text-gray-600 leading-relaxed mb-3">
                  相手もこのアプリを使っていれば、メールとアプリのダブル通知で安心。登録時に相手のIDを入力するか、QRコードで新規登録してもらうだけです。
                </p>
      ...
      {showQrModal && qrDataUrl && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-8">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center">
            ...
            <p className="text-xs text-gray-400 mb-4">このQRコードを読み取ると新規登録画面が開きます</p>
